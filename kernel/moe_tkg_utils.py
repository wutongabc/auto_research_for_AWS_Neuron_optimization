...
def broadcast_token_affinity(
    ...
    if dims.T == 1:
        # Token zero is already in the first partition channel. Broadcast its
        # gathered Top-K row directly and avoid an intermediate shuffle tile.
        stream_shuffle_broadcast(
            src=token_affinities[0:1, : dims.K],
            dst=dst,
        )
        return
