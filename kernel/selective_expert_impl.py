...
"""Specialize the production selective-expert TKG kernel for fused init."""
...
_new = '''            output_accum = output_temp[0 : dims.H0, 0 : dims.H1_shard, local_token_idx]
            if params.expert_params.expert_affinities_scaling_mode == ExpertAffinityScaleMode.POST_SCALE:
                if expert_k_idx == 0:
                    # Initialize the accumulator with the scaled first expert,
                    # avoiding a separate down_sb -> output_temp tensor copy.
                    nisa.tensor_scalar(
                        dst=output_accum,
                        data=down_sb,
                        op0=nl.multiply,
