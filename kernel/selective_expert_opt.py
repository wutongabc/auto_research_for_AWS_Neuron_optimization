...
def _selective_expert_moe_tkg(
    ...
    # Keep all expert down results in one regular allocation. The expert axis is
    # selected away before projection, preserving the expected 2-D view.
    down_output = sbm.alloc_stack(
        (dims.H0, dims.H1_shard, dims.K),
        dtype=io_dtype,
        name="down_output_sbuf",
        buffer=nl.sbuf,
    )
    down_output_tv = safe_tensor_view(down_output)
    ...
    for local_token_idx in range(T_per_shard):
        ...
        for expert_k_idx in range(dims.K):
            ...
            # Down projection
            down_sb_view = down_output_tv.select(dim=2, index=expert_k_idx)
            down_sb = down_sb_view.get_view()
