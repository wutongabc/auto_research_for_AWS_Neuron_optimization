...
def run_gate_up_projection_lhs_rhs_swap(
    ...
    if not params.skip_gate_proj:
        combined_shape = (dims.I0, num_I_tiles * 2, dims.T)
        ...
        gate_up_view = TensorView(gate_up_sb_fp32)
        gate_sb_view = gate_up_view.slice(
            dim=1, start=0, end=num_I_tiles
        )
        up_sb_view = gate_up_view.slice(
            dim=1, start=num_I_tiles, end=num_I_tiles * 2
        )
    else:
    ...
    if dims.num_shards > 1:
        recv_shape = (
            ...
            if gate_up_sb_fp32 != None
            else up_sb_view.get_view().shape
        )
    ...
    return tiles, gate_sb_view, up_sb_view, gate_up_recv, gate_up_sb_fp32
